...
def save_all_source_code(output_path='_utils/output.txt', excluded_dirs=None, excluded_files=None, included_ext=None):
    if excluded_dirs is None:
        excluded_dirs = {
            ...
            'node_modules', '.next', '.git', '.venv', '.vscode', '.idea', 'dist', 'build', 'data', 'output', 'logs','lib', 'models'}
    if excluded_files is None:
        excluded_files = {
            os.path.basename(output_path),
            '.gitignore', 'pyproject.toml', 'credentials.py', 'read_file_for_gpt.py', '.env', 'dump.txt'
            'package-lock.json'
        }
    if included_ext is None:
        included_ext = {'.py', '.js', '.ts', '.jsx', '.tsx', '.json', '.html', '.css', '.yaml', 'sh', 'yml'}
